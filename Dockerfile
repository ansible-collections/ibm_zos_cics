...
COPY ${SRC} ${ROOT}
RUN apt-get update && apt-get install -y gnupg2 && apt-get update && apt-get install -y git python3-pip openssh-client ansible && pip3 install --no-cache-dir -r ${ROOT}/requirements.txt && pip3 install ansible-lint yamllint pylint pytest-ansible virtualenv bandit
