...
class ActionModule(ActionBase):
    def run(self, tmp=None, task_vars=None):
        ...
        if (
            region_data_sets.get("dfhintra", None) is None
            or region_data_sets.get("dfhintra").get("dsn", None) is None
        ):
            if region_data_sets.get("template", None) is None:
                return {
                    "failed": True,
                    "changed": False,
                    "msg": "Specify either template or dfhintra in region_data_sets",
                }
            dsn = template_dsn(
                _templar=self._templar,
                task_vars=task_vars,
                var_name="data_set_name",
                replace_val="DFHINTRA",
        ...
        return self._execute_module(
            module_name="ibm.ibm_zos_cics.intrapartition",
