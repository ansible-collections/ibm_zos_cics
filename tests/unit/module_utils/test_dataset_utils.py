...
from ansible_collections.ibm.ibm_zos_cics.tests.unit.helpers.data_set_helper import (
    ...
    RMUTL_stdout,
    get_job_dd_output,
    get_job_dd_output_execution,
    get_sample_job_output_with_content
...
def test__get_job_output():
    ...
    expected_result = get_sample_job_output_with_content()
    ...
    data_set_utils._get_job_dd = MagicMock(side_effect=[
        get_job_dd_output("JESMSGLG", job_id),
        get_job_dd_output("JESJCL", job_id),
        get_job_dd_output("JESSYSMSG", job_id),
        get_job_dd_output("SYSPRINT", job_id)
    ])
...
def test__get_job_output_multiple_jobs_failure():
    ...
    expected_executions = []
    expected_exception = MVSExecutionException(
        "Query for job status for {0} with job ID {1} returned more than one result. Jobs returned: ".format(
            job_name,
            job_id
        ), expected_executions
    )
    ...
    assert e.value.executions == expected_executions
    assert expected_exception.message in e.value.message


def test__get_job_dd():
    job_id = "JOB12345"
    dd_name = "DD Name"
    rc = 0
    stdout = RMUTL_stdout("AUTOINIT", job_id)
    stderr = "CC"
    mock_dd_execution = [get_job_dd_output_execution(rc, dd_name, job_id)]
    data_set_utils._execute_command = MagicMock(return_value=(rc, stdout, stderr))
...
def test__get_job_dd_failure():
    ...
    expected_message = f"RC 8 when getting job output for DD Name from {job_id}"
